            ...
            <ul className="flex flex-wrap gap-4 justify-center md:justify-start md:gap-6">
              {[
                { name: "Home", icon: "fa-house", link: "/" },
                {
                  ...
                  link: "/shop",
                  dropdown: ["Shop 1", "Shop 2"],
                },
                {
                  name: "Bakery",
                  icon: "fa-cake-candles",
                  link: "/bakery",
                  dropdown: ["Bake 1", "Bake 2"],
                },
                {
                  name: "Beverages",
                  icon: "fa-mug-hot",
                  link: "/beverages",
                  dropdown: ["Category 1", "Category 2"],
                },
                ...
                <li
                  key={index}
                  className="relative text-white px-4 py-2 rounded-full bg-[#5A9B48] hover:bg-[#4a8b3c] cursor-pointer transition-colors"
                      ...
                      {activeDropdown === name.toLowerCase() && (
                        <ul className="absolute left-0 mt-2 bg-white border border-gray-200 rounded-md shadow-lg z-50 w-48">
                          {dropdown.map((item, i) => (
                            <li
                              key={i}
                              className="px-4 py-2 bg-[#72BF4F] text-white hover:bg-[#5a9b48] transition-colors rounded-t-md"
                            >
                              <Link
                                to={`${link}/${item
                                  .replace(" ", "")
          ...
          <div className="h-full flex flex-col">
            <ul className="flex-1 overflow-y-auto">
              {categories.slice(0, 9).map((category, index) => (
                <li
                  key={index}
                  className="px-4 py-2 bg-[#72BF4F] text-white hover:bg-[#5a9b48] transition-colors rounded-md"
                >
      ...
      <section className="flex flex-col md:flex-row">
        <div
          className={`w-full sm:w-1/4 ${
            showCategories ? "block" : "hidden sm:hidden"
          }`}
        >
            ...
            {[
              {
                ...
                description:
                  "Explore the best places to eat with our guided tours.",
                buttonText: "Learn More",
              },
              {
                src: image2,
                alt: "Image 2",
                title: "Find Your Dream Food",
              ...
              },
              {
                ...
                description: "Discover foods that will take your breath away.",
